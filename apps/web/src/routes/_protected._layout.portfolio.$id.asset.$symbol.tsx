...
export default function AssetDetailPage() {
  const { id: portfolioId, symbol } = useParams<{ id: string, symbol: string }>();
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-zinc-500 animate-pulse">Loading asset details...</p>
        </div>
      </div>
    );
  }
  ...
  if (isError || !assetData) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center p-8 text-center">
        <div className={`mb-4 rounded-full p-4 ${isNoTransactions ? 'bg-amber-500/10 text-amber-500' : 'bg-rose-500/10 text-rose-500'}`}>
  ...
  const isStale = new Date().getTime() - new Date(details.last_updated).getTime() > 3600000; // 1 hour
        ...
        <div className="mx-auto max-w-7xl">
          <Breadcrumb className="mb-4">
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild>
                  <Link to={`/portfolio/${portfolioId}`}>{portfolio?.name || 'Portfolio'}</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {isStale && (
                     <Badge variant="destructive" className="bg-amber-500/10 text-amber-500 border-amber-500/20 flex gap-1">
                        <Clock className="h-3 w-3" /> Stale
                     </Badge>
                  )}
                </div>
                <div className="flex items-center gap-4 mt-1 text-sm">
                   <span className="text-zinc-500">{symbol} • {details.market}</span>
                   <span className="h-1 w-1 rounded-full bg-zinc-700" />
                   <span className="text-zinc-400 font-medium">Last Price: {formatCurrency(details.current_price)}</span>
                </div>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <Button variant="outline" className="border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10" onClick={() => setShowMethodology(!showMethodology)}>
        ...
        <div className="mx-auto max-w-7xl space-y-8">
          ...
          <AnimatePresence>
            {showMethodology && (
              ...
              >
                <MethodologyPanel 
                   calculationMethod={details.calculation_method as any || CalculationMethod.WEIGHTED_AVG} 
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
             ...
             <div className="flex flex-col gap-4">
               <div className="rounded-2xl border border-white/5 bg-zinc-900/50 p-0 shadow-xl overflow-hidden flex flex-col h-[500px]">
                    ...
                    <Table>
                      <TableHeader className="bg-white/[0.01]">
                        <TableRow className="border-white/5 hover:bg-transparent">
                          <TableHead className="text-zinc-500 pl-5">Date</TableHead>
                          <TableHead className="text-zinc-500">Type</TableHead>
                          <TableHead className="text-zinc-500 text-right pr-5">Amount</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {transactions.map((tx) => (
                          <TableRow key={tx.id} className="border-white/5 hover:bg-white/[0.04] transition-colors group">
                            <TableCell className="pl-5 text-zinc-400 text-xs py-4">
                                {new Date(tx.date).toLocaleDateString()}
                            </TableCell>
                            <TableCell>
                               <span className={`inline-flex items-center gap-1.5 font-medium text-xs px-2 py-0.5 rounded-full ${
                            ...
                            <TableCell className="text-right pr-5">
                               <div className="text-sm font-medium text-white">{tx.quantity} units</div>
                               <div className="text-[10px] text-zinc-500">@ {formatCurrency(tx.price)}</div>
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                  ...
                  <Link to={`/portfolio/${portfolioId}/transactions`} className="p-4 text-center text-xs font-medium text-zinc-500 hover:text-emerald-400 border-t border-white/5 bg-white/[0.01] transition-colors">
...
function StatCard({ label, value, percentage, subValue, type = 'default' }: { 
  label: string, 
  value: string, 
  percentage?: number, 
  subValue?: string,
  ...
  return (
    <div className="relative group rounded-2xl border border-white/5 bg-zinc-900/50 p-5 shadow-lg transition-all hover:bg-zinc-900/80 hover:border-white/10 overflow-hidden">
      ...
      <p className="text-xs font-medium text-zinc-500 mb-2 uppercase tracking-tight">{label}</p>
      <div className="flex items-baseline gap-2">
        <span className="text-2xl font-bold text-white tracking-tight">{value}</span>
      </div>
      
      {type === 'dynamic' && percentage !== undefined && (
        <div className={`mt-2 flex items-center gap-1 text-sm font-medium ${isPositive ? 'text-emerald-400' : 'text-rose-400'}`}>
      ...
      {type === 'neutral' && (
        <p className="mt-2 text-xs text-zinc-600 font-medium italic">Breakdown of FX impact (FR7)</p>
      )}
    </div>
  );
}
