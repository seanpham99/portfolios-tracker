...
# Add dags directory to path (works both locally and in Docker)
...
class TestSendTelegramMessage:
    ...
    @responses.activate
    def test_success_notification_sent(self, sample_airflow_context, mock_environment_variables):
    ...
    def test_missing_credentials_skips_notification(self, sample_airflow_context, monkeypatch):
...
class TestNotificationCallbacks:
    ...
    @patch('etl_modules.notifications.send_telegram_message')
...
class TestSendTelegramNewsSummary:
    ...
    @responses.activate
    @patch('etl_modules.notifications.summarize_news_with_gemini')
    def test_sends_ai_summary_when_available(self, mock_gemini, mock_environment_variables):
...
class TestGetLatestStockData:
    ...
    @patch('etl_modules.notifications.clickhouse_connect.get_client')
    def test_successful_data_fetch(self, mock_get_client, mock_environment_variables):
        ...
        mock_market_result.result_rows = [(
            'HPG', '2024-12-22', 25500.0, 5000000, 2.5,
            25000.0, 24000.0, 45.2, 100.5, 95.3, 5.2,
            8.5, 'Steel', 'Basic Materials'
        )]
    ...
    @patch('etl_modules.notifications.clickhouse_connect.get_client')
    def test_connection_error_handled(self, mock_get_client, mock_environment_variables):
...
class TestSummarizeNewsWithGemini:
    ...
    @responses.activate
    @patch('etl_modules.notifications.get_latest_stock_data')
    def test_successful_summary_generation(self, mock_get_data, mock_environment_variables):
