...
const components = {
  ...
  h2: (props: any) => (
    <h2 className="mt-10 scroll-m-20 border-b border-zinc-800 pb-1 text-3xl font-semibold tracking-tight text-white first:mt-0" {...props} />
  ),
  h3: (props: any) => (
  ...
  a: (props: any) => {
    ...
    if (isInternal) {
      return (
        <Link href={props.href} className="font-medium text-emerald-500 underline underline-offset-4 hover:text-emerald-400">
          {props.children}
        </Link>
      );
    }
  ...
  ul: (props: any) => (
    <ul className="my-6 ml-6 list-disc [&>li]:mt-2 text-zinc-300" {...props} />
  ),
  ol: (props: any) => (
  ...
  code: (props: any) => (
    <code className="relative rounded bg-zinc-900 px-[0.3rem] py-[0.2rem] font-mono text-sm font-semibold text-zinc-300" {...props} />
  ),
  img: (props: any) => (
    ...
    <div className="container mx-auto px-4 md:px-6 py-24">
      ...
      <div className="max-w-3xl mx-auto mt-16 pt-8 border-t border-white/10 text-center">
        <p className="text-zinc-400 mb-4">
            Ready to start tracking your wealth?
        </p>
        <Link href="/signup" className="inline-flex h-10 items-center justify-center rounded-full bg-emerald-600 px-8 text-sm font-medium text-white transition-colors hover:bg-emerald-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
            Create Free Account
        </Link>
      </div>
    </div>
