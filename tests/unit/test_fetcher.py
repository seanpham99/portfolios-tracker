...
class TestFetchStockPrice:
    ...
    @patch('etl_modules.fetcher.Quote')
...
class TestFetchFinancialRatios:
    ...
    @patch('etl_modules.fetcher.Finance')
...
class TestFetchIncomeStmt:
    ...
    def test_income_stmt_success(self, mock_finance_class):
        ...
        from etl_modules.fetcher import fetch_income_stmt
        result = fetch_income_stmt('HPG')

        assert not result.empty
        assert set(['ticker','fiscal_date','year','quarter','revenue','cost_of_goods_sold','gross_profit','operating_profit','net_profit_post_tax']).issubset(result.columns)
        assert result['ticker'].iloc[0] == 'HPG'
...
class TestFetchDividends:
    ...
    @patch('etl_modules.fetcher.Company')
    def test_dividends_success(self, mock_company_class):
        ...
        from etl_modules.fetcher import fetch_dividends
        result = fetch_dividends('HPG')

        assert not result.empty
        assert set(['ticker','exercise_date','cash_year','cash_dividend_percentage','stock_dividend_percentage','issue_method']).issubset(result.columns)
        assert result['ticker'].iloc[0] == 'HPG'
...
class TestFetchNews:
    ...
    def test_news_success(self, mock_company_class):
        ...
        assert not result.empty
        assert set(['ticker','publish_date','title','source','price_at_publish','price_change','price_change_ratio','rsi','rs','news_id']).issubset(result.columns)
        assert result['ticker'].iloc[0] == 'HPG'
        assert pd.to_datetime(result['publish_date'].iloc[0]).strftime('%Y-%m-%d') == '2024-12-20'
