...
import { Bell, X, Check, TrendingUp, Wallet, BarChart3, Info } from "lucide-react"
            ...
            >
              <GlassCard className="overflow-hidden" hover={false}>
                ...
                <div className="flex items-center justify-between border-b border-white/[0.08] px-4 py-3">
                  <h3 className="font-serif text-lg font-medium text-white">Notifications</h3>
                  <div className="flex items-center gap-2">
                    {unreadCount > 0 && (
                      <button
                        onClick={() => portfolioStore.markAllNotificationsRead()}
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((notification) => (
                      <motion.div
                        ...
                        onClick={() => portfolioStore.markNotificationRead(notification.id)}
                      >
                        <div
                          className={cn(
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <p
                              className={cn("text-sm font-medium", notification.read ? "text-zinc-400" : "text-white")}
                            >
                              {notification.title}
                            </p>
                            {!notification.read && <span className="h-2 w-2 shrink-0 rounded-full bg-indigo-500" />}
                          </div>
                          <p className="mt-0.5 text-xs text-zinc-500 line-clamp-2">{notification.message}</p>
                          <p className="mt-1 text-[10px] text-zinc-600">{formatTimeAgo(notification.timestamp)}</p>
                        </div>
                      </motion.div>
                    ))
                  )}
