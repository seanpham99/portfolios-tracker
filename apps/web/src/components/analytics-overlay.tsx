...
export function AnalyticsOverlay({ isOpen, onClose, stages, netWorthData, insights }: AnalyticsOverlayProps) {
        ...
        >
          <div className="min-h-screen p-8">
            ...
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h1 className="font-serif text-4xl font-light text-white">Portfolio Intelligence</h1>
                <p className="mt-2 text-zinc-500">Real-time analytics and AI-powered insights</p>
              </div>
              <button
                onClick={onClose}
              ...
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-all ${
                    activeTab === id ? "bg-white/10 text-white" : "text-zinc-500 hover:bg-white/5 hover:text-zinc-300"
                  }`}
                >
              ...
              <div className="grid gap-6 lg:grid-cols-3">
                ...
                <GlassCard className="p-6 lg:col-span-2" hover={false}>
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-medium text-white">Net Worth Trajectory</h3>
                  </div>
                  <div className="flex h-48 items-center justify-center">
                    <Sparkline data={netWorthData} color="#10b981" width={600} height={160} strokeWidth={2.5} />
                  </div>
                  <div className="mt-4 flex items-end justify-between">
                    ...
                    <div
                      className={`rounded-full px-3 py-1 text-sm font-medium ${
                        metrics.ytdReturn >= 0 ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400"
                      }`}
                    >
                      {metrics.ytdReturn >= 0 ? "+" : ""}
                    ...
                    <div className="flex items-center justify-between rounded-xl bg-white/[0.03] p-3">
                      ...
                      <span
                        className={`text-lg font-semibold tabular-nums ${
                          metrics.sharpeRatio >= 1 ? "text-emerald-400" : "text-amber-400"
                        }`}
                      >
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <GlassCard className="p-6" hover={false}>
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-medium text-white">vs Benchmarks</h3>
                  </div>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between rounded-xl bg-emerald-500/10 p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-zinc-500">YTD Performance</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 text-xl font-semibold text-emerald-400">
                        <ArrowUpRight className="h-5 w-5" />+{metrics.ytdReturn.toFixed(1)}%
                      </div>
                    </div>
                    {Object.entries(benchmarkData).map(([name, { return: ret, color }]) => (
                      <div key={name} className="flex items-center justify-between rounded-xl bg-white/[0.03] p-4">
                        <div className="flex items-center gap-3">
                          <div
                        ...
                        <div
                          className={`flex items-center gap-1 text-lg font-semibold ${
                            ret >= metrics.ytdReturn ? "text-zinc-400" : "text-zinc-500"
                          }`}
                        >
                          {ret >= 0 ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownRight className="h-4 w-4" />}
                          {ret >= 0 ? "+" : ""}
                          {ret}%
                        </div>
                      </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <div className="mb-2 flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium text-white">{metrics.sharpeRatio.toFixed(2)}</span>
                      </div>
                      <div className="relative h-3 rounded-full bg-white/5">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${Math.min((metrics.sharpeRatio / 3) * 100, 100)}%` }}
                    ...
                    <div>
                      <div className="mb-2 flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium text-white">{metrics.volatility.toFixed(1)}%</span>
                      </div>
                      <div className="h-3 overflow-hidden rounded-full bg-white/5">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${Math.min((metrics.volatility / 50) * 100, 100)}%` }}
                    ...
                    <div>
                      ...
                      <div className="h-3 overflow-hidden rounded-full bg-white/5">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${Math.min((metrics.maxDrawdown / 30) * 100, 100)}%` }}
                    ...
                    <div className="rounded-xl bg-white/[0.03] p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-sm text-zinc-400">Portfolio Beta</p>
                          <p className="text-xs text-zinc-600">vs S&P 500</p>
                        </div>
                        <span className="text-2xl font-semibold tabular-nums text-indigo-400">1.24</span>
                      </div>
                    </div>
                  </div>
                </GlassCard>
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <GlassCard className="p-6" hover={false}>
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-medium text-white">Asset Classes</h3>
                  </div>
                  <div className="space-y-4">
                    {stages.map((stage, i) => {
                  ...
                  <div className="mt-8 flex justify-center">
                    <svg width="200" height="200" viewBox="0 0 200 200">
                      ...
                      <text x="100" y="115" textAnchor="middle" className="fill-zinc-500 text-xs">
                        Total Value
                      </text>
                    </svg>
                  </div>
                ...
                <GlassCard className="p-6" hover={false}>
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <h3 className="text-lg font-medium text-white">Equity Sectors</h3>
                  </div>
                  <div className="space-y-3">
                    {sectorBreakdown.map((sector, i) => (
                      ...
                      >
                        <div className="w-24 text-sm text-zinc-400">{sector.name}</div>
                        <div className="flex-1 h-8 overflow-hidden rounded-lg bg-white/5">
                          ...
                          >
                            <span className="text-xs font-medium text-white">{sector.percentage}%</span>
                          </motion.div>
                        </div>
                      </motion.div>
                    ))}
                  ...
                  {/* Top Holdings */}
                  <div className="mt-8">
                    <h4 className="mb-4 text-sm font-medium text-zinc-400">Top Holdings</h4>
                      ...
                      {equityStage?.assets.slice(0, 5).map((asset, i) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium text-zinc-300">{asset.symbol}</p>
