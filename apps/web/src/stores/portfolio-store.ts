...
export interface Notification {
  id: string
  type: "price_alert" | "portfolio_change" | "market_movement" | "system" | "asset_request"
  title: string
  message: string
  timestamp: Date
  read: boolean
...
const initialNotifications: Notification[] = [
  {
    ...
    type: "price_alert",
    title: "NVDA up 5%+",
    message: "NVIDIA Corp. has risen 5.67% today, exceeding your alert threshold.",
...
const initialSettings: Settings = {
  currency: "USD",
  refreshInterval: 30,
  notifications: { priceAlerts: true, dailySummary: true, significantMovements: true },
}
...
export const portfolioStore = {
    ...
    const newStages = state.stages.map((stage) => {
      const newAssets = stage.assets.map((asset) => {
        ...
        const newSparkline = [
          ...
          asset.sparklineData[asset.sparklineData.length - 1] * (1 + priceChange / 100),
        ]
        return { ...asset, value: newValue, change: newChange, sparklineData: newSparkline }
      })
      const totalValue = newAssets.reduce((sum, a) => sum + a.value, 0)
      const avgChange = newAssets.reduce((sum, a) => sum + a.change, 0) / newAssets.length
  ...
  submitAssetRequest(request: Omit<AssetRequest, "id" | "requestedAt" | "status" | "progress" | "requestedBy">) {
  ...
  updateAssetRequestStatus(id: string, status: AssetRequest["status"], progress?: number) {
    const assetRequests = state.assetRequests.map((r) => {
      if (r.id === id) {
        ...
        if (status === "processing" && !r.estimatedCompletion) {
          updated.estimatedCompletion = new Date(Date.now() + 1000 * 60 * 60 * 4) // 4 hours
        }
...
if (typeof window !== "undefined") {
  setInterval(() => {
    const processingRequests = state.assetRequests.filter((r) => r.status === "processing")
    processingRequests.forEach((request) => {
      if (request.progress < 95) {
        portfolioStore.updateAssetRequestProgress(request.id, request.progress + Math.random() * 5)
      }
    })
  }, 3000)
}
...
export function usePortfolioStore() {
  return useSyncExternalStore(portfolioStore.subscribe, portfolioStore.getSnapshot, portfolioStore.getServerSnapshot)
}
...
export function useAssetRequests() {
  const store = usePortfolioStore()
  const pendingCount = store.assetRequests.filter((r) => r.status === "pending" || r.status === "processing").length
  return {
