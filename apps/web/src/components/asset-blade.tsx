...
export function AssetBlade({ asset, index, onClick }: AssetBladeProps) {
  ...
  return (
    <GlassCard className="flex w-[280px] shrink-0 flex-col gap-4 p-5" delay={index * 0.1} onClick={onClick}>
      {/* Header */}
      ...
      <div className="flex justify-center py-2">
        <Sparkline data={asset.sparklineData} color={isPositive ? "#10b981" : "#f43f5e"} width={240} height={50} />
      </div>
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-500">{asset.allocation}% of portfolio</p>
        </div>
      </div>
    </GlassCard>
  )
